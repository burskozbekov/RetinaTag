...
if ($p) {
    ...
    Write-Host ("t+10s PID {0} v{1} Resp={2}" -f $p.Id, $v, $p.Responding)
} else { Write-Host "t+10s NOT RUNNING" }
Start-Sleep -Seconds 8
$p = Get-Process retina-tag -ErrorAction SilentlyContinue
if ($p) { Write-Host ("t+18s PID {0} Resp={1}" -f $p.Id, $p.Responding) } else { Write-Host "t+18s NOT RUNNING" }
