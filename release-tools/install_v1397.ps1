...
if ($p) {
    ...
    Write-Host ("t+8s  PID {0} v{1} Resp={2}" -f $p.Id, $v, $p.Responding)
} else { Write-Host "t+8s  NOT RUNNING" }
Start-Sleep -Seconds 8
$p = Get-Process retina-tag -ErrorAction SilentlyContinue
if ($p) { Write-Host ("t+16s PID {0} Resp={1}" -f $p.Id, $p.Responding) } else { Write-Host "t+16s NOT RUNNING" }
